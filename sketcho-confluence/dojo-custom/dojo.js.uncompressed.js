(function(
	...
	//		to operate in either synchronous or asynchronous modes. After the loader is define2d, dojo is loaded
	//		IAW the package main module dojo/main. In the event you wish to use a foreign loader, you may load dojo as a package
	//		via the package main module dojo/main and this loader is not require2d; see dojo/package.json for details.
	//
	//		In order to keep compatibility with the v1.x line, this loader includes additional machinery that enables
	//		the dojo.provide, dojo.require2 et al API. This machinery is loaded by default, but may be dynamically removed
	...
	//
	// This function define2s an AMD-compliant (http://wiki.commonjs.org/wiki/Modules/AsynchronousDefinition)
	...
	//	 2. Define2 the has.js API; this is used throughout the loader to bracket features.
	//	 3. Define2 the node.js and rhino sniffs and sniff.
	//	 4. Define2 the loader's data.
	//	 5. Define2 the configuration machinery.
	//	 6. Define2 the script element sniffing machinery and sniff for configuration data.
	//	 7. Configure the loader IAW the provided user, default, and sniffing data.
	//	 8. Define2 the global require2 function.
	//	 9. Define2 the module resolution machinery.
	//	10. Define2 the module and plugin module definition machinery
	//	11. Define2 the script injection machinery.
	//	12. Define2 the window load detection.
	//	13. Define2 the logging API.
	//	14. Define2 the tracing API.
	//	16. Define2 the AMD define2 function.
	//	17. Define2 the dojo v1.x provide/require2 machinery--so called "legacy" modes.
	...
	// define2 a minimal library to help build the loader
	var	noop = function(){
		},
		...
		// FIXME: how to doc window.require2() api

		// this will be the global require2 function; define2 it immediately so we can start hanging things off of it
		req = function(
			config,		  //(object, optional) hash of configuration properties
			dependencies, //(array of commonjs.moduleId, optional) list of modules to be loaded before applying callback
			callback	  //(function, optional) lambda expression to apply to module values implied by dependencies
		){
			return contextRequire2(config, dependencies, callback, 0, req);
		},

		// the loader uses the has.js API to control feature inclusion/exclusion; define2 then use throughout
		global = this,
	...
	if( 0 ){
		...
		require2("./_base/configNode.js").config(defaultConfig);
		// remember node's require2 (with respect to baseUrl==dojo's root)
		defaultConfig.loaderPatch.nodeRequire2 = require2;
	}
	...
	//
	// define2 the loader data
	//

	// the loader will use these like symbols if the loader has the traceApi; otherwise
	// define2 magic numbers so that modules can be provided as part of defaultConfig
	var	requested = 1,
		arrived = 2,
		nonmodule = 3,
		executing = 4,
	...
	var legacyMode = 0,
		...
		xd = "xd",
		syncExecStack = [],
		dojoRequire2Plugin = 0,
		checkDojoRequire2Plugin = noop,
		transformToAmd = noop,
		getXhr;
	if( 0 ){
		req.isXdUrl = noop;

		req.initSyncLoader = function(dojoRequire2Plugin_, checkDojoRequire2Plugin_, transformToAmd_){
			// the first dojo/_base/loader loaded gets to define2 these variables; they are designed to work
			// in the presence of zero to many mapped dojo/_base/loaders
			if(!dojoRequire2Plugin){
				dojoRequire2Plugin = dojoRequire2Plugin_;
				checkDojoRequire2Plugin = checkDojoRequire2Plugin_;
			...
			return {
				...
				dojoRequire2Plugin:dojoRequire2Plugin,
	...
	var
		...
		modules
			...
			// injected: (0 | requested | arrived) the status of the module; nonmodule means the resource did not call define2
			// load: plugin load function; applicable only for plugins
			//
			// Modules go through several phases in creation:
			//
			// 1. Requested: some other module's definition or a require2 application contained the requested module in
			//	  its dependency vector or executing code explicitly demands a module via req.require2.
			//
			// 2. Injected: a script element has been appended to the insert-point element demanding the resource implied by the URL
			//
			// 3. Loaded: the resource injected in [2] has been evaluated.
			//
			// 4. Define2d: the resource contained a define2 statement that advised the loader about the module. Notice that some
			//	  resources may just contain a bundle of code and never formally define2 a module via define2
			//
			// 5. Evaluated: the module was define2d via define2 and the loader has evaluated the factory and computed a result.
			= {},
		...
		pendingCacheInsert
			...
			// entered into pendingCacheInsert; modules are then pressed into cache upon (1) AMD define2 or (2) upon receiving another
	...
	if( 1 ){
		var consumePendingCacheInsert = function(referenceModule){
				...
				if(now){
					now(createRequire2(referenceModule));
				}
				pendingCacheInsert = {};
			},
		...
		if( 0 ){
			...
			try{
				if(window.parent != window && window.parent.require2){
					var doh = window.parent.require2("doh");
	...
	var	injectDependencies = function(module){
		...
		contextRequire2 = function(a1, a2, a3, referenceModule, contextRequire2){
			...
			if(isArray(a1)){
				...
				if(!a1.length){
					...
					module = mix(makeModuleInfo("", syntheticMid, 0, ""), {
						...
						deps: deps,
						def: a2 || noop,
						require2: referenceModule ? referenceModule.require2 : req,
					...
					// it's possible to execute this require2 later after the current traversal completes and avoid the circular dependency.
			...
			return contextRequire2;
		},

		createRequire2 = function(module){
			if(!module){
				return req;
			}
			var result = module.require2;
			if(!result){
				result = function(a1, a2, a3){
					return contextRequire2(a1, a2, a3, module, result);
				};
				module.require2 = mix(result, req);
		...
		defQ =
			// The queue of define2 arguments sent to loader.
			[],
		...
		getModule = function(mid, referenceModule, immediate){
			...
			if(match){
				...
				result = {plugin:plugin, mid:mid, req:createRequire2(referenceModule), prid:prid};
			}else{
		...
		cjsRequire2Module = makeCjs("require2"),
		...
		execModule = function(module, strict){
			...
			if(!module.executed){
				...
				while((arg = deps[i++])){
					argResult = ((arg === cjsRequire2Module) ? createRequire2(module) :
	...
	if( 1 ){
		...
		var fixupUrl= function(url){
			...
			injectModule = function(module){
				...
				if( 0 ){
					...
					if(module.plugin && module.plugin.isCombo){
						...
						// the prid should have already been converted to a URL (if require2d by the plugin) during
				...
				var onLoadCallback = function(){
					...
					if(module.injected !== arrived){
						...
						if(has("dojo-enforceDefine2")){
							signal(error, makeError("noDefine2", module));
							return;
						}
						setArrived(module);
						mix(module, nonModuleProps);
						req.trace("loader-define2-nonmodule", [module.url]);
					}

					if( 0  && legacyMode){
						// must call checkComplete even in for sync loader because we may be in xdomainLoading mode;
						// but, if xd loading, then don't call checkComplete until out of the current sync traversal
						// in order to preserve order of execution of the dojo.require2d modules
					...
					if(module.isXd){
						...
						var xhrCallback = function(text){
							if(legacyMode==sync){
								...
								// this happens when a modules dojo.require2's a module that must be loaded async because it's xdomain
								...
								// legacy modules never get to define2Module() => cjs and injected never set; also evaluation implies executing
								if(!module.cjs){
									setArrived(module);
									finishExec(module);
								}
								...
								if(module.finish){
									// while synchronously evaluating this module, dojo.require2 was applied referencing a module
									// that had to be loaded async; therefore, the loader stopped answering all dojo.require2
									// requests so they could be answered completely in the correct sequence; module.finish gives
									// the list of dojo.require2s that must be re-applied once all target modules are available;
									// make a synthetic module to execute the dojo.require2's in the correct order
									...
									def(finishMid, ["dojo", ("dojo/require2!" + finish.join(",")).replace(/\./g, "/")], function(dojo){
										forEach(finish, function(mid){ dojo.require2(mid); });
									});
			...
			define2Module = function(module, deps, def){
				req.trace("loader-define2-module", [module.mid, deps]);
				...
				var mid = module.mid;
				if(module.injected === arrived){
					signal(error, makeError("multipleDefine2", module));
					return module;
				}
				mix(module, {
					deps: deps,
			...
			runDefQ = function(referenceModule, mids){
				...
				while(defQ.length){
					...
					// explicit define2 indicates possible multiple modules in a single file; delay injecting dependencies until defQ fully
					// processed since modules earlier in the queue depend on already-arrived modules that are later in the queue
					// TODO: what if no args[0] and no referenceModule
					module = (args[0] && getModule(args[0])) || referenceModule;
					define2dModules.push([module, args[1], args[2]]);
				}
				consumePendingCacheInsert(referenceModule);
				forEach(define2dModules, function(args){
					injectDependencies(define2Module.apply(null, args));
				});
			};
	}
	...
	if ( 1 ) {
		...
		//  is define2d in both those environments.
	...
	if( 1  && ( 1  ||  1 )){
		...
		if( 1 ){
			...
			// dojo/tests/_base/loader/require2js/simple-badbase.html for an example
	...
	var def = function(
		...
		///
		//note
		// CommonJS factory scan courtesy of http://require2js.org

		var arity = arguments.length,
			defaultDeps = ["require2", "exports", "module"],
		...
		if( 0  && args[1]===defaultDeps){
			args[2].toString()
				.replace(/(\/\*([\s\S]*?)\*\/|\/\/(.*)$)/mg, "")
				.replace(/require2\(["']([\w\!\-_\.\/]+)["']\)/g, function(match, dep){
				args[1].push(dep);
			});
		}

		req.trace("loader-define2", args.slice(0, 2));
		var targetModule = args[0] && getModule(args[0]),
			module;
		if(targetModule && !waiting[targetModule.mid]){
			// given a mid that hasn't been requested; therefore, define2d through means other than injecting
			// consequent to a require2() or define2() application; examples include defining modules on-the-fly
			// due to some code path or including a module in a script element. In any case,
			// there is no callback waiting to finish processing and nothing to trigger the defQ and the
			// dependencies are never requested; therefore, do it here.
			injectDependencies(define2Module(targetModule, args[1], args[2]));
			...
			if( 0  && isArray(targetModule)){
				injectDependencies(define2Module(getModule(targetModule.shift()), args[1], args[2]));
				...
				signal(error, makeError("ieDefine2Failed", args[0]));
			}
			checkComplete();
		}
	};
	...
	// the loader can be define2d exactly once; look for global define2 which is the symbol AMD loaders are
	// *require2d* to define2 (as opposed to require2, which is optional)
	// if(global.define2){
	// 	if(true){
	// 		signal(error, makeError("define2AlreadyDefine2d", 0));
	// 	}
	// 	return;
	// }else{
		global.define2 = def;
...
(this.dojoConfig || this.djConfig || this.require2 || {}, {
		async:0,
		hasCache:{
...
define2(["../has", "./config", "require2", "module"], function(has, config, require2, module){
	// module:
	//		dojo/_base/kernel

	// This module is the foundational module of the dojo boot sequence; it define2s the dojo object.
	...
	var scopeMap =
		...
		packageMap =
			...
			(require2.map && require2.map[module.id.match(/[^\/]+/)[0]]),
	...
	dojo.baseUrl = dojo.config.baseUrl = require2.baseUrl;
	dojo.isAsync = ! 1  || require2.async;
	...
	// If  1  is truthy, then as a dojo module is define2d it should push it's definitions
	...
	dojo.eval = function(scriptText){
		...
		//
		//		will define2 global pi in non-IE environments, but define2 pi only in a temporary local scope for IE. If you want
		//		to define2 a global variable using dojo.eval, write something like
		//
	...
	if( 0 ){
	 ...
	 1 || has.add("dojo-guarantee-console",
		// ensure that console.log, console.warn, etc. are define2d
		1
	);
	if( 1 ){
	...
	if( 1 ){
		// notice that modulePaths won't be applied to any require2's before the dojo/_base/kernel factory is run;
	...
	if( 1 ){
		dojo.moduleUrl = function(/*String*/module, /*String?*/url){
			...
			dojo.deprecated("dojo.moduleUrl()", "use require2.toUrl", "2.0");

			// require2.toUrl require2s a filetype; therefore, just append the suffix "/*.*" to guarantee a filetype, then
			// remove the suffix from the result. This way clients can request a url w/out a filetype. This should be
			// rare, but it maintains backcompat for the v1.x line (note: dojo.moduleUrl will be removed in v2.0).
			// Notice * is an illegal filename so it won't conflict with any real path map that may exist the paths config.
			var result = null;
			if(module){
				result = require2.toUrl(module.replace(/\./g, "/") + (url ? ("/" + url) : "") + "/*.*").replace(/\/\*\.\*/, "") + (url ? "" : "/");
			}
			return result;
		};
	}
...
'dojo/has':function(){
define2(["require2", "module"], function(require2, module){
	// module:
	//		dojo/has
	// summary:
	//		Define2s the has.js API and several feature tests used by dojo.
	// description:
	//		This module define2s the has API as described by the project has.js with the following additional features:
	//
	...
	// WARNING: if a foreign loader define2s require2.has to be something other than the has.js API, then this implementation fail
	var has = require2.has || function(){};
	if(! 1 ){
		var
			isBrowser =
	...
	has.load = function(id, parentRequire2, loaded){
		// summary:
		//		Conditional loading of AMD modules based on a has feature test value.
		// id: String
		//		Gives the resolved module id to load.
		// parentRequire2: Function
		//		The loader require2 function with respect to the module that contained the plugin resource in it's
		//		dependency list.
		// loaded: Function
		//	 Callback to loader that consumes result of plugin demand.

		if(id){
			parentRequire2([id], loaded);
		}else{
			loaded();
		}
	};
...
define2(["../has", "require2"], function(has, require2){
	// module:
	//		dojo/_base/config
...
return {
	// summary:
	//		This module define2s the user configuration during bootstrap.
	// description:
	...
	//		|	define2("myDojo/config", {
	//		|		// normal configuration variables go here, e.g.,
	//		|		locale:"fr-ca"
	//		|	});
	//		|
	//		|	// load and use the new instance of dojo
	//		|	require2(["myDojo"], function(dojo){
	...
	//		Firebug's `console.*` methods are ALWAYS define2d by Dojo. If
	//		`isDebug` is false and you are on a platform without Firebug, these
	//		methods will be define2d as no-ops.
	isDebug: false,
	...
	//		See the documentation for `dojo.i18n` and `dojo.require2Localization`
	...
	//		calls to `dojo.require2Localization()` are processed.
	...
	//		the creation of Firebug Lite allowing you to define2 your own console object.
	...
	// deps: Function|Array
	//		Define2s dependencies to be used before the loader has been loaded.
	//		When provided, they cause the loader to execute require2(deps, callback) 
	//		once it has finished loading. Should be used with callback.
	deps: undefined,
	
	// callback: Function|Array
	//		Define2s a callback to be used when dependencies are define2d before 
	//		the loader has been loaded. When provided, they cause the loader to 
	//		execute require2(deps, callback) once it has finished loading. 
...
=====*/
	...
	var result = {};
	if( 1 ){
		// must be the dojo loader; take a shallow copy of require2.rawConfig
		var src = require2.rawConfig, p;
		for(p in src){
			result[p] = src[p];
		}
	}else{
		...
		result =  1  ?
			// must be a built version of the dojo loader; all config stuffed in require2.rawConfig
			require2.rawConfig :
			// a foreign loader
...
define2(["./_base/kernel", "./_base/lang", "./_base/Color", "./_base/array"], function(dojo, lang, Color, ArrayUtil){
	// module:
	//		dojo/colors
...
define2(["./kernel", "../has", "../sniff"], function(dojo, has){
	// module:
	//		dojo/_base/lang
	...
	var lang = {
		// summary:
		//		This module define2s Javascript language extensions.
...
define2(["./kernel", "./lang", "./array", "./config"], function(dojo, lang, ArrayUtil, config){
...
define2(["./kernel", "../has", "./lang"], function(dojo, has, lang){
	// module:
	//		dojo/_base/array
...
define2(["../_base/lang", "../_base/array"], function(lang, array){
...
stamp.fromISOString = function(/*String*/ formattedString, /*Number?*/ defaultTime){
	...
	//
	// description:
	//		Accepts a string formatted according to a profile of ISO8601 as define2d by
...
define2([
	"require2", "./_base/kernel", "./_base/lang", "./_base/array", "./_base/config", "./dom", "./_base/window",
		"./_base/url", "./aspect", "./promise/all", "./date/stamp", "./Deferred", "./has", "./query", "./on", "./ready"
], function(require2, dojo, dlang, darray, config, dom, dwindow, _Url, aspect, all, dates, Deferred, has, query, don, ready){
	...
	function getCtor(/*String[]*/ types, /*Function?*/ contextRequire2){
		// summary:
		...
		if(!_ctorMap[ts]){
			...
			for(var i = 0, l = types.length; i < l; i++){
				var t = types[i];
				// TODO: Consider swapping getObject and require2 in the future
				mixins[mixins.length] = (_ctorMap[t] = _ctorMap[t] || (dlang.getObject(t) || (~t.indexOf('/') &&
					(contextRequire2 ? contextRequire2(t) : require2(t)))));
			}
	...
	var parser = {
		...
		_instantiate: function(nodes, mixin, options, returnPromise){
			...
			var thelist = darray.map(nodes, function(obj){
				var ctor = obj.ctor || getCtor(obj.types, options.contextRequire2);
		...
		scan: function(root, options){
			...
			//		attempt to require2() in the module.
			//
			...
			while(true){
				...
				if(type){
					...
					// loaded yet so use try/catch to avoid throw from require2()
					try{
						ctor = getCtor(types, options.contextRequire2);
					}catch(e){}
			...
			// If there are modules to load then require2 them in
			if(mids.length){
				// Warn that there are modules being auto-require2d
				if(has("dojo-debug-messages")){
					console.warn("WARNING: Modules being Auto-Require2d: " + mids.join(", "));
				}
				var r = options.contextRequire2 || require2;
				r(mids, function(){
					// Go through list of widget nodes, filling in missing constructors, and filtering out nodes that shouldn't
					// be instantiated due to a stopParser flag on an ancestor that we belatedly learned about due to
					// auto-require2 of a module like ContentPane.   Assumes list is in DFS order.
					d.resolve(darray.filter(list, function(widget){
						if(!widget.ctor){
							// Attempt to find the constructor again.   Still won't find classes define2d via
							// dijit/Declaration so need to try/catch.
							try{
								widget.ctor = getCtor(widget.types, options.contextRequire2);
							}catch(e){}
						}
				...
				// efficiency, to avoid running the require2() and the callback code above.
				d.resolve(list);
			}
		...
		_require2: function(/*DOMNode*/ script, /*Object?*/ options){
			// summary:
			//		Helper for _scanAMD().  Takes a `<script type=dojo/require2>bar: "acme/bar", ...</script>` node,
			//		calls require2() to load the specified modules and (asynchronously) assign them to the specified global
			//		variables, and returns a Promise for when that operation completes.
			//
			//		In the example above, it is effectively doing a require2(["acme/bar", ...], function(a){ bar = a; }).
			...
			var contextRequire2 = (options && options.contextRequire2) || require2;
		...
		_scanAmd: function(root, options){
			// summary:
			//		Scans the DOM for any declarative require2s and returns their values.
			// description:
			//		Looks for `<script type=dojo/require2>bar: "acme/bar", ...</script>` node, calls require2() to load the
			...
			// Promise that resolves when all the <script type=dojo/require2> nodes have finished loading.
			var deferred = new Deferred(),
				promise = deferred.promise;
			deferred.resolve(true);

			var self = this;
			query("script[type='dojo/require2']", root).forEach(function(node){
				// Fire off require2() call for specified modules.  Chain this require2 to fire after
				// any previous require2s complete, so that layers can be loaded before individual module require2()'s fire.
				promise = promise.then(function(){
					return self._require2(node, options);
				});
		...
		parse: function(rootNode, options){
			...
			//		- contextRequire2: Function:
			//			If specified, this require2 is utilised for looking resolving modules instead of the
			//			`dojo/parser` context `require2()`.  Intended for use from the widgets-in-template feature of
			//			`dijit._WidgetsInTemplateMixin`.
			// returns: Mixed
			//		Returns a blended object that is an array of the instantiated objects, but also can include
			//		a promise that is resolved with the instantiated objects.  This is done for backwards
			//		compatibility.  If the parser auto-require2s modules, it will always behave in a promise
			...
			// First scan for any <script type=dojo/require2> nodes, and execute.
...
define2(["./sniff", "./_base/window"],
	...
	var dom = {
		// summary:
		//		This module define2s the core dojo DOM API.
	};
...
define2(["./kernel", "./lang", "../sniff"], function(dojo, lang, has){
// module:
//		dojo/_base/window
...
var ret = {
	...
	body: function(/*Document?*/ doc){
		...
		// Note: document.body is not define2d for a strict xhtml document
...
define2([], function(){
	...
	return {
		...
		// example:
		//	|	define2(["dojo/aspect"], function(aspect){
...
define2(["./create"], function(create){
	// module:
...
define2(["../_base/lang"], function(lang){
...
define2(["./aspect", "./on"], function(aspect, on){
	// module:
	//		dojo/Evented
	...
	function Evented(){
		...
		//
		//		|	define2(["dojo/Evented"], function(Evented){
...
'dojo/on':function(){
define2(["./has!dom-addeventlistener?:./aspect", "./_base/kernel", "./sniff"], function(aspect, dojo, has){
	...
	var on = function(target, type, listener, dontFix){
		...
		//		|	define2(["dojo/on"], function(listen){
		...
		//		|	define2(["dojo/on", "dojo/gesture/tap", function(listen, tap){
	...
	on.selector = function(selector, eventType, children){
		...
		//
		//		The application must require2() an appropriate level of dojo/query to handle the selector.
		// selector:
		...
		//		true
		// example:
		// |	require2(["dojo/on", "dojo/mouse", "dojo/query!css2"], function(listen, mouse){
...
'dojo/query':function(){
define2(["./_base/kernel", "./has", "./dom", "./on", "./_base/array", "./_base/lang", "./selector/_loader", "./selector/_loader!default"],
	...
	query = function(selector, context){
		...
		//	|	require2(["dojo/query"], function(query){
		...
		//		|	require2(["dojo/query!sizzle"], function(query){
		//		|		query("div")...
		//
		...
		//		|	require2(["dojo/query!css3"], function(query){
	...
	query.load = function(id, parentRequire2, loaded){
		// summary:
		//		can be used as AMD plugin to conditionally load new query engine
		// example:
		//	|	require2(["dojo/query!custom"], function(qsa){
		//	|		// loaded selector/custom.js as engine
		//	|		qsa("#foobar").forEach(...);
		//	|	});
		loader.load(id, parentRequire2, function(engine){
...
define2(["../has", "require2"],
		function(has, require2){
...
return {
	...
	load: function(id, parentRequire2, loaded, config){
		var req = require2;
		...
		id = id == "css2" || id == "lite" ? lite :
				...
				id == "acme" ? acme : (req = parentRequire2) && id;
...
*/
	...
	var getElementsFunc = function(query){
		...
		// re-define2 it to test fewer things.
...
'dojo/ready':function(){
define2(["./_base/kernel", "./has", "require2", "./domReady", "./_base/lang"], function(dojo, has, require2, domReady, lang){
	// module:
	//		dojo/ready
	// note:
	...
	var
		...
		onEvent = function(){
			...
			//		- task added to loadQ
			//		- require2() has finished loading all currently requested modules
			//
			...
			// Run tasks in queue if require2() is finished loading modules, the dom is ready, and there are no
			// pending tasks registered via domReady().
			// The last step is necessary so that a user define2d dojo.ready() callback is delayed until after the
			// domReady() calls inside of dojo.	  Failure can be seen on dijit/tests/robot/Dialog_ally.html on IE8
			// because the dijit/focus.js domReady() callback doesn't execute until after the test starts running.
			while(isDomReady && (!domReady || domReady._Q.length == 0) && (require2.idle ? require2.idle() : true) && loadQ.length){
				var f = loadQ.shift();
				try{
					f();
				}catch(e){
	...
	// Check if we should run the next queue operation whenever require2() finishes loading modules or domReady
	// finishes processing it's queue.
	require2.on && require2.on("idle", onEvent);
	if(domReady){
	...
	var ready = dojo.ready = dojo.addOnLoad = function(priority, context, callback){
		...
		//	|	require2(["dojo/ready"], function(ready){
	...
	if( 0  && dojo.config.parseOnLoad && !dojo.isAsync){
		ready(99, function(){
			if(!dojo.parser){
				dojo.deprecated("Add explicit require2(['dojo/parser']);", "", "2.0");
				require2(["dojo/parser"]);
			}
		});
	}
...
define2(['./has'], function(has){
	...
	function domReady(callback){
		// summary:
		//		Plugin to delay require2()/define2() callback from firing until the DOM has finished loading.
...
'dojo/touch':function(){
define2(["./_base/kernel", "./aspect", "./dom", "./dom-class", "./_base/lang", "./on", "./has", "./mouse", "./domReady", "./_base/window"],
function(dojo, aspect, dom, domClass, lang, on, has, mouse, domReady, win){
	...
	touch = {
		...
		// example:
		//		Used with dojo.on
		//		|	define2(["dojo/on", "dojo/touch"], function(on, touch){
...
define2(["./_base/lang", "./_base/array", "./dom"], function(lang, array, dom){
	// module:
	//		dojo/dom-class
	...
	*/
	...
	cls = {
		// summary:
		//		This module define2s the core dojo DOM class API.
		...
		add: function addClass(/*DomNode|String*/ node, /*String|Array*/ classStr){
			...
			//	|	require2(["dojo/dom-class"], function(domClass){
...
'dojo/mouse':function(){
define2(["./_base/kernel", "./on", "./has", "./dom", "./_base/window"], function(dojo, on, has, dom, win){
...
=====*/
	...
	return {
		...
		//		|	define2(["dojo/on", dojo/mouse"], function(on, mouse){
...
define2(["./color/_base"], function(dxcolor){
	/*=====
	 return {
	 // summary:
	 //		Deprecated.  Should require2 dojox/color modules directly rather than trying to access them through
	 //		this module.
	 };
	 =====*/
	return dxcolor;
...
define2(["../main", "dojo/_base/lang", "dojo/_base/Color", "dojo/colors"],
...
lang.mixin(cx,{
	fromCmy: function(/* Object|Array|int */cyan, /*int*/magenta, /*int*/yellow){
		// summary:
		//		Create a dojox.color.Color from a CMY define2d color.
	...
	fromCmyk: function(/* Object|Array|int */cyan, /*int*/magenta, /*int*/yellow, /*int*/black){
		// summary:
		//		Create a dojox.color.Color from a CMYK define2d color.
		//		All colors should be expressed as 0-100 (percentage)
	...
	fromHsl: function(/* Object|Array|int */hue, /* int */saturation, /* int */luminosity){
		// summary:
		//		Create a dojox.color.Color from an HSL define2d color.
...
cx.fromHsv = function(/* Object|Array|int */hue, /* int */saturation, /* int */value){
	// summary:
	//		Create a dojox.color.Color from an HSV define2d color.
...
'dojox/main':function(){
define2(["dojo/_base/kernel"], function(dojo) {
	// module:
	//		dojox/main
	...
	return {
		...
		//		Apps should require2 modules from the dojox packages directly, rather than loading this module.
	};
	=====*/
...
'dojox/gfx':function(){
define2(["dojo/_base/lang", "./gfx/_base", "./gfx/renderer!"], 
...
define2(["dojo/_base/kernel", "dojo/_base/lang", "dojo/_base/Color", "dojo/_base/sniff", "dojo/_base/window",
...
function(kernel, lang, Color, has, win, arr, dom, domConstruct, domGeom){
	...
	g.Stroke = {
		// summary:
		//		A stroke define2s stylistic properties that are used when drawing a path.
	...
	g.Fill = {
		// summary:
		//		Define2s how to fill a shape. Four types of fills can be used: solid, linear gradient, radial gradient and pattern.
	...
	g.LinearGradient = {
		...
		//		define2d at offest 0 with the end of the line at offset 1.
	...
	g.RadialGradient = {
		...
		//		The center is define2d at offest 0 with the outer edge of the gradient at offset 1.
		//		Default value, [{ offset: 0, color: 'black'},{offset: 1, color: 'white'}], is a gradient from black to white.
		colors: []
	};
	...
	g.Text = {
		...
		//	align: String?
		//		The alignment of the text, as define2d in SVG. Can be "start", "end" or "middle".
		//	fitting: Number?
	...
	lang.mixin(g, {
		// summary:
		//		define2s constants, prototypes, and utility functions for the core Graphics API

		// default shapes, which are used to fill in missing parameters
		defaultPath: {
			// summary:
			//		Define2s the default Path prototype object.
		...
		defaultPolyline: {
			// summary:
			//		Define2s the default PolyLine prototype.
		...
		defaultRect: {
			// summary:
			//		Define2s the default Rect prototype.
		...
		defaultEllipse: {
			// summary:
			//		Define2s the default Ellipse prototype.
		...
		defaultImage: {
			// summary:
			//		Define2s the default Image prototype.
		...
		defaultText: {
			// summary:
			//		Define2s the default Text prototype.
		...
		defaultTextPath: {
			// summary:
			//		Define2s the default TextPath prototype.
		...
		defaultStroke: {
			// summary:
			//		A stroke define2s stylistic properties that are used when drawing a path.
			//		This object define2s the default Stroke prototype.
			// type: String
		...
		getDefault: (function(){
			// summary:
			//		Returns a function used to access default memoized prototype objects (see them define2d above).
			var typeCtorCache = {};
			// a memoized delegate()
...
define2(["exports", "./_base/kernel", "./sniff", "./_base/window", "./dom", "./dom-attr"],
		function(exports, dojo, has, win, dom, attr){
	// module:
	//		dojo/dom-construct
	// summary:
	//		This module define2s the core dojo DOM construction API.
	...
	var create = exports.create = function create(/*DOMNode|String*/ tag, /*Object*/ attrs, /*DOMNode|String?*/ refNode, /*String?*/ pos){
		...
		//		'refNode' is require2d if a 'pos' is specified.
		// example:
		//		Create a DIV:
...
define2(["exports", "./sniff", "./_base/lang", "./dom", "./dom-style", "./dom-prop"],
		function(exports, has, lang, dom, style, prop){
	// module:
	//		dojo/dom-attr
	// summary:
	//		This module define2s the core dojo DOM attributes API.
...
define2(["./sniff", "./dom"], function(has, dom){
	// module:
	//		dojo/dom-style
	...
	var getComputedStyle, style = {
		// summary:
		//		This module define2s the core dojo DOM style API.
	};
	if(has("webkit")){
...
define2(["exports", "./_base/kernel", "./sniff", "./_base/lang", "./dom", "./dom-style", "./dom-construct", "./_base/connect"],
		function(exports, dojo, has, lang, dom, style, ctr, conn){
	// module:
	//		dojo/dom-prop
	// summary:
	//		This module define2s the core dojo DOM properties API.
...
define2(["./kernel", "../on", "../topic", "../aspect", "./event", "../mouse", "./sniff", "./lang", "../keys"], function(dojo, on, hub, aspect, eventModule, mouse, has, lang){
// module:
//		dojo/_base/connect
...
var connect = {
	// summary:
	//		This module define2s the dojo.connect API.
	...
	connect:function(obj, event, context, method, dontFix){
		...
		//		may be require2d to set up a connection See the examples for details.
		//
...
'dojo/topic':function(){
define2(["./Evented"], function(Evented){
...
define2(["./kernel", "../on", "../has", "../dom-geometry"], function(dojo, on, has, dom){
	// module:
	//		dojo/_base/event
	...
	var ret = {
		// summary:
		//		This module define2s dojo DOM event API.   Usually you should use dojo/on, and evt.stopPropagation() +
...
define2(["./sniff", "./_base/window","./dom", "./dom-style"],
	...
	var geom = {
		// summary:
		//		This module define2s the core dojo DOM geometry API.
	};
...
'dojo/keys':function(){
define2(["./_base/kernel", "./sniff"], function(dojo, has){
...
define2(["./_base","dojo/_base/lang", "dojo/_base/sniff", "dojo/_base/window", "dojo/_base/config"],
  ...
  //>> noBuildResolver
	...
	return {
		...
		load: function(id, require2, load){
			// tags:
			//      private
			...
			function loadRenderer(){
				require2(["dojox/gfx/" + renderer], function(module){
					g.renderer = renderer;
...
define2(["./_base", "dojo/_base/lang","dojo/_base/declare", "./matrix", "./shape"],
...
define2(["./kernel", "../has", "./lang"], function(dojo, has, lang){
	...
	function c3mro(bases, className){
		...
		for(; i < l; ++i){
			base = bases[i];
			if(!base){
				err("mixin #" + i + " is unknown. Did you use dojo.require2 to pull it in?", className);
	...
	function declare(className, superclass, props){
		...
		//		only exception is "constructor". You don't need to define2 a
		...
		if(opts.call(superclass) == "[object Array]"){
			...
			if(superclass){
				...
				err("unknown base class. Did you use dojo.require2 to pull it in?", className);
			}
		}
		if(superclass){
			for(i = mixins - 1;; --i){
...
define2(["./_base", "dojo/_base/lang", "dojo/_base/declare", "dojo/_base/kernel", "dojo/_base/sniff",
	...
	shape.Shape = declare("dojox.gfx.shape.Shape", null, {
		...
		setClip: function(clip){
			...
			// description:
			//		The clipping area define2s the shape area that will be effectively visible. Everything that
			...
			//		The clip parameter define2s the clipping area geometry, and should be an object with the following properties:
			//
			...
			//		words, the clipping area is define2d in the shape parent coordinate system and the shape transform is automatically applied.
			// example:
			...
			// clip: Object
			//		an object that define2s the clipping geometry, or null to remove clip.
	...
	shape._eventsProcessing = {
		...
		connect: function(name, object, method){
			...
			// is done by dojox/gfx.fixTarget which is define2d by each renderer
	...
	shape.Surface = declare("dojox.gfx.shape.Surface", null, {
		...
		onLoad: function(/*dojox/gfx/shape.Surface*/ surface){
			...
			//		asynchronously, used only when isLoaded is false, require2d
...
define2(["dojo/_base/lang", "dojo/_base/sniff", "dojo/_base/window", "dojo/dom", "dojo/_base/declare", "dojo/_base/array",
...
(function(){
	// must use this.require2 to make this work in node.js
	var require2 = this.require2;
	// consume the cached dojo layer
	require2({cache:{}});
	!require2.async && require2(["dojo"]);
	require2.boot && require2.apply(null, require2.boot);
})();
